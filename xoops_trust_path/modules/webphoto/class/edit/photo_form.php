...
class webphoto_edit_photo_form extends webphoto_edit_form
{
    ...
    /**
     * webphoto_edit_photo_form constructor.
     * @param $dirname
     * @param $trust_dirname
     */
     ...
     * @return \webphoto_edit_form|\webphoto_edit_photo_form|\webphoto_lib_element|\webphoto_lib_error|\webphoto_lib_form
     */
    ...
    {
        $template = 'db:' . $this->_DIRNAME . '_form_photo.tpl';
        ...
        $arr = array_merge($this->build_form_base_param(), $this->build_form_photo($item_row), $param);
    ...
    public function build_form_photo($item_row)
    {
        ...
        list($show_detail_div, $show_detail_div_on) = $this->show_detail_div($show_detail_div_mode);
    ...
    public function build_form_common($is_edit)
    {
        ...
        list($photo_url, $show_file_photo_delete) = $this->build_file_url(_C_WEBPHOTO_FILE_KIND_CONT, 'item_external_url');

        list($jpeg_url, $show_file_jpeg_delete) = $this->build_file_url(_C_WEBPHOTO_FILE_KIND_JPEG, 'item_external_thumb');

        list($show_thumb_dsc_select, $show_thumb_dsc_embed) = $this->show_thumb_dsc();

        list($item_codeinfo_select_options, $item_codeinfo_hiddens) = $this->item_codeinfo_param();

        list($item_perm_read_input_checkboxs, $item_perm_read_list, $item_perm_read_hiddens) = $this->item_perm_read_param();

        list($item_perm_down_input_checkboxs, $item_perm_down_list, $item_perm_down_hiddens) = $this->item_perm_down_param();
    ...
    public function show_item_embed()
    {
        ...
        return [$show_type, $show_text, $show_src];
    }
    ...
    public function show_item_icon_name()
    {
        ...
        $jpeg_url = $this->build_file_url_by_kind($item_row, _C_WEBPHOTO_FILE_KIND_JPEG);
        $thumb_url = $this->build_file_url_by_kind($item_row, _C_WEBPHOTO_FILE_KIND_THUMB);
    ...
    public function item_file_array($is_edit)
    {
        ...
        foreach ($this->_ini_file_list as $file) {
            $kind_name = mb_strtoupper('_C_WEBPHOTO_FILE_KIND_' . $file);
    ...
    {
        return $this->sanitize($this->_tag_build_class->tag_name_array_to_str($tag_name_array));
    }
    ...
    {
        $type = $this->get_row_by_key('item_embed_type');
        if ($this->is_embed_type() && (_C_WEBPHOTO_EMBED_NAME_GENERAL == $type)) {
            return true;
        }
    ...
    {
        return $this->sanitize(formatTimestamp(time(), _WEBPHOTO_DTFMT_YMDHI));
    }
    ...
    public function init_editor()
    {
        ...
        $arr = $this->_editor_class->init_form($editor, $name1, $name1, $value1, $this->get_ini('submit_item_description_rows'), $this->get_ini('submit_item_description_cols'));
