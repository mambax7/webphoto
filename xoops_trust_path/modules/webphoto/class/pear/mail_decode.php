...
class webphoto_pear_mail_decode
{
    ...
    public function decode_headers($headers, $charset)
    {
        ...
        if (isset($param['from'])) {
            list($param['from_name'], $param['from_addr']) = $this->parse_name_addr($param['from']);
        }

        if (isset($param['to'])) {
            list($param['to_name'], $param['to_addr']) = $this->parse_name_addr($param['to']);
        }

        if (isset($param['reply_to'])) {
            list($reply_name, $param['reply_to']) = $this->parse_name_addr($param['reply_to']);
        }

        if (isset($param['return_path'])) {
            list($return_name, $param['return_path']) = $this->parse_name_addr($param['return_path']);
        }
