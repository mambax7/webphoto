...
class webphoto_main_place extends webphoto_show_list
{
    ...
    /**
     * webphoto_main_place constructor.
     * @param $dirname
     * @param $trust_dirname
     */
    ...
    public function list_get_photo_list()
    {
        ...
        foreach ($list_rows as $row) {
            ...
            if ($place) {
                ...
                $photo_rows = $this->_public_class->get_rows_by_place_array_orderby($place_arr, $this->_PHOTO_LIST_ORDER, $this->_PHOTO_LIST_LIMIT);
            } else {
                ...
                $photo_rows = $this->_public_class->get_rows_by_place_orderby($place_not, $this->_PHOTO_LIST_ORDER, $this->_PHOTO_LIST_LIMIT);
            }
            ...
            if ($total > 0) {
                $arr[] = $this->list_build_photo_array($title, $param, $total, $photo_row);
    ...
    public function list_build_detail($place_in)
    {
        ...
        // if not set place
        if (_C_WEBPHOTO_PLACE_STR_NOT_SET == $place) {
            $title = $this->get_constant('PLACE_NOT_SET');
