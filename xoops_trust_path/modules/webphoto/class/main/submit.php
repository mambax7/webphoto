...
class webphoto_main_submit extends webphoto_edit_submit
{
    ...
    public function _check($action)
    {
        ...
        if (!$ret) {
            redirect_header($this->get_redirect_url(), $this->get_redirect_time(), $this->get_redirect_msg());
            exit();
        }
    }
    ...
    public function _submit_bulk()
    {
        ...
        for ($i = 1; $i <= $this->_MAX_PHOTO_FILE; ++$i) {
            ...
            $item_row = $this->_factory_create_class->build_item_row_photo($item_row, $file_tmp_name, $file_media_name);
    ...
    public function _video()
    {
        ...
        list($url, $time, $msg) = $this->build_redirect($this->build_submit_redirect_param(!$ret, $item_row['item_cat_id']));
    ...
    public function _build_form_preview()
    {
        ...
        // set preview name
        list($item_row, $image_info) = $this->_build_preview_info($item_row);
