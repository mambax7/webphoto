...
class webphoto_main_gmap_location extends webphoto_base_this
{
    ...
    /**
     * webphoto_main_gmap_location constructor.
     * @param $dirname
     * @param $trust_dirname
     */
    public function __construct($dirname, $trust_dirname)
    {
        ...
        $this->_TEMPLATE = 'db:' . $dirname . '_main_gmap_location.tpl';
     ...
     * @return \webphoto_lib_error|\webphoto_main_gmap_location
     */
    ...
    public function _build_list_location()
    {
        ...
        if (!is_array($item_row) || !count($item_row)) {
            return [$show_gmap, $gmap_list];
        }

        // narrow area
        $item_rows_narrow = $this->_get_item_rows_by_gmap_area($item_row, $this->_AREA_NARROW_NS, $this->_AREA_NARROW_EW);
        ...
        if (!$flag_narrow || (count($item_rows_narrow) < $this->_AREA_NUM)) {
            $item_rows_wide = $this->_get_item_rows_by_gmap_area($item_row, $this->_AREA_WIDE_NS, $this->_AREA_WIDE_EW);
    ...
    public function _get_item_rows_by_gmap_area($item_row, $ns, $ew)
    {
        ...
        return $this->_public_class->get_rows_by_gmap_area($id, $lat, $lon, $ns, $ew, $this->_AREA_NUM, 0, true);
    }
