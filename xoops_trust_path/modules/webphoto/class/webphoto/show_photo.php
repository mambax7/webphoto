...
class webphoto_show_photo extends webphoto_base_this
{
    ...
    /**
     * webphoto_show_photo constructor.
     * @param $dirname
     * @param $trust_dirname
     */
     ...
     * @return \webphoto_lib_error|\webphoto_show_photo
     */
    ...
    public function build_photo_show_basic($row, $tag_name_array = null)
    {
        ...
        list($desc_disp, $summary) = $this->build_show_desc_summary($row, $this->_flag_highlight, $this->_keyword_array);

        $cont_disp = $this->build_show_content($row, $this->_flag_highlight, $this->_keyword_array);
        ...
        list($cont_size, $cont_duration) = $this->get_show_file_size_duration($show_arr, _C_WEBPHOTO_FILE_KIND_CONT);
    ...
    public function build_photo_show($row)
    {
        ...
        list($is_newphoto, $is_updatedphoto) = $this->build_show_is_new_updated($item_time_update, $item_status);
        ...
        $cat_row = $this->_catHandler->get_cached_row_by_id($item_cat_id);
        if (is_array($cat_row)) {
    ...
    {
        if ($size > 0) {
            return $this->_utility_class->format_filesize($size, $this->_ini_filesize_precision);
        }
    ...
    public function build_show_desc_summary($row, $flag_highlight = false, $keyword_array = null)
    {
        ...
        if (is_array($keyword_array)) {
            ...
            $summary = $this->_multibyte_class->build_summary_with_search($summary, $keyword_array, $this->_cfg_item_summary);

        // more than limit
        } elseif ($this->_multibyte_class->str_len($desc) > $this->_cfg_item_summary) {
            $summary = $this->_multibyte_class->build_summary($desc, $this->_cfg_item_summary);
    ...
    {
        $str = $this->_multibyte_class->build_summary_with_search($row['item_content'], $keyword_array, $this->_ini_content_length);
    ...
    {
        $str = $row['item_exif'];
        if (mb_strlen($str) > $this->_ini_exif_length) {
    ...
    public function build_show_is_new_updated($time_update, $status)
    {
        ...
        return [$is_newphoto, $is_updatedphoto];
    }
    ...
    {
        return $this->_utility_class->format_time($time, $this->get_constant('HOUR'), $this->get_constant('MINUTE'), $this->get_constant('SECOND'));
    }
    ...
    {
        if (0 == $this->_cfg_perm_cat_read) {
            return true;
        }

        return $this->_catHandler->is_cached_public_read_in_all_parents_by_id($row['item_cat_id']);
    }
