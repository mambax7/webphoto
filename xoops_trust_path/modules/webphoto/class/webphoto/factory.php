...
class webphoto_factory extends webphoto_base_this
{
    ...
    public $_xoops_header_array = [];
    public $_show_array = [];
    public $_template_param = [];
     ...
     * @return \webphoto_factory|\webphoto_lib_error
     */
    ...
    {
        $name1 = 'main_' . $mode . '.tpl';
        $name2 = 'main_index.tpl';
    ...
    {
        if (is_array($arr)) {
            $this->_template_param = array_merge($this->_template_param, $arr);
        }
    ...
    public function show_check_timeline_category($cat_id)
    {
        ...
        if (_C_WEBPHOTO_TIMELINE_SCALE_NONE == $scale) {
            return false;
        }
    ...
    {
        $show = false;
        if ((_C_WEBPHOTO_DISPLAYTYPE_IMAGE == $row['item_displaytype'])
            && (_C_WEBPHOTO_DETAIL_ONCLICK_LIGHTBOX == $row['item_detail_onclick'])) {
            $show = true;
        }
