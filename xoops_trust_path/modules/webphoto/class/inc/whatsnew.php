...
class webphoto_inc_whatsnew extends webphoto_inc_public
{
    ...
    /**
     * webphoto_inc_whatsnew constructor.
     * @param $dirname
     * @param $trust_dirname
     */
    ...
    public function whatsnew($limit = 0, $offset = 0)
    {
        ...
        foreach ($item_rows as $item_row) {
            ...
            list($image, $width, $height) = $this->build_img_url($item_row, $this->_SHOW_IMAGE, $this->_SHOW_ICON);
     ...
     * @return bool
     */
    public function _is_gmap($row)
    {
        if ((0 != (float)$row['item_gmap_latitude'])
            || (0 != (float)$row['item_gmap_longitude'])
            || (0 != (int)$row['item_gmap_zoom'])) {
            return true;
        }
