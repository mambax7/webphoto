...
class webphoto_inc_blocks extends webphoto_inc_public
{
    ...
    /**
     * webphoto_inc_blocks constructor.
     * @param $dirname
     * @param $trust_dirname
     */
    public function __construct($dirname, $trust_dirname)
    {
        ...
        $this->_block_id = isset($_GET['bid']) ? (int)$_GET['bid'] : 0;
    ...
    public function catlist_show($options)
    {
        ...
        list($cols, $width) = $this->_catlist_class->calc_width($cols);
    ...
    {
        $template = 'db:' . $this->_DIRNAME . '_block_' . $mode . '.tpl';
        $tpl = new XoopsTpl();
    ...
    public function timeline_show($options)
    {
        ...
        if ($this->_init_timeline) {
            ...
            list($photos, $photo_num) = $this->_build_photos($options, $item_rows, false);

            if ($photo_num > 0) {
                $this->_timeline_class->set_show_onload(true);
                $tl_param = $this->_timeline_class->fetch_timeline('painter', $scale, $this->_TIMELINE_DATE, $photos);
    ...
    public function _top_show_common($mode, $options)
    {
        ...
        list($show_popbox_js, $popbox_js) = $this->_get_popbox_js($mode, $use_popbox);
        ...
        if (!$tpl->is_cached($template) || (0 == $cache_time) || $show_popbox_js) {
    ...
    public function _build_block($mode, $options)
    {
        ...
        list($photos, $photo_num) = $this->_build_photos($options, $item_rows);

        $block['photo'] = $photos;
        $block['photo_num'] = $photo_num;

        list($show_gmap, $gmap) = $this->_build_gmap_block($mode, $block['photo'], $options);
     ...
     * @return string
     */
    public function _top_edit_gmap_mode($options)
    {
     ...
     * @return string
     */
    public function _build_gmap_iframe($num)
    {
        $src = $this->_MODULE_URL . '/index.php?fct=gmap_location&amp;block_id=' . (int)$this->_block_id . '&amp;option_num=' . (int)$num;
     ...
     * @return null|string
     */
    public function build_form_select($name, $value, $options, $size = 1)
    {
        if (!is_array($options) || !count($options)) {
            return null;
        }
     ...
     * @param null $default
     */
    public function _get_option($options, $num, $default = null)
    {
     ...
     * @return int
     */
    public function _get_option_int($options, $num, $default = 0)
    {
     ...
     * @return float
     */
    public function _get_option_float($options, $num, $default = 0)
    {
        $val = $this->_get_option($options, $num, $default);
     ...
     * @return int
     */
    public function _get_option_cols($options, $num)
    {
     ...
     * @return array
     */
    public function _build_imgsrc($item_row)
    {
        $img_photo_src = '';
        $img_photo_width = 0;
        $img_photo_height = 0;
        ...
        list($cont_url, $cont_width, $cont_height) = $this->build_show_file_image($cont_row);

        list($thumb_url, $thumb_width, $thumb_height) = $this->build_show_file_image($thumb_row);

        list($large_url, $large_width, $large_height) = $this->build_show_file_image($large_row);

        list($small_url, $small_width, $small_height) = $this->build_show_file_image($small_row);
        ...
        list($img_thumb_width, $img_thumb_height) = $this->_adjust_image_thumb($img_thumb_width, $img_thumb_height);
     ...
     * @return bool
     */
    public function _is_src_image_kind($kind)
    {
     ...
     * @param $item_row
     */
    public function _build_cat_title($item_row)
    {
     ...
     * @return string
     */
    public function _build_short_title($str, $max)
    {
        if (0 == $max) {
            $str = '';
        } elseif (mb_strlen($str) >= $max) {
     ...
     * @return string
     */
    public function _build_hits_suffix($hits)
    {
        $val = $hits > 1 ? 'hits' : 'hit';
     ...
     * @return array
     */
    public function _adjust_image_thumb($width, $height)
    {
        return $this->_adjust_image_size($width, $height, $this->_cfg_thumb_width, $this->_cfg_thumb_height);
    }
     ...
     * @return array
     */
    public function _adjust_image_size($width, $height, $max_width, $max_height)
    {
        if ($width > $max_width) {
            $mag = $max_width / $width;
            $width = $max_width;
     ...
     * @return array
     */
    public function _build_gmap_block($mode, $photos, $options)
    {
        if (!$this->_check_gmap($mode)) {
            return [false, null];
        }
     ...
     * @return bool
     */
    public function _exist_gmap($photo)
    {
        if (0 != $photo['item_gmap_latitude']) {
            return true;
        }
        if (0 != $photo['item_gmap_longitude']) {
            return true;
        }
        if (0 != $photo['item_gmap_zoom']) {
            return true;
        }
     ...
     * @return bool
     */
    public function _check_gmap($mode)
    {
     ...
     * @return string
     */
    public function _shorten_text($str, $max)
    {
     ...
     * @return array|null
     */
    public function _get_item_rows_timeline($latest, $random)
    {
        $param = [
            'cat_limitation' => 0,
        ...
        $latest_rows = $this->get_item_rows_for_block($param, $this->_ORDERBY_LATEST, $latest, $this->_OFFSET_ZERO, $this->_KEY_TRUE);

        $random_rows = $this->get_item_rows_for_block($param, $this->_ORDERBY_RANDOM, $random, $this->_OFFSET_ZERO, $this->_KEY_TRUE);
     ...
     * @return array|bool
     */
    public function _get_item_rows_top_common($mode, $options)
    {
        $photos_num = $this->_get_option_int($options, 1, 5);
        $cat_limitation = $this->_get_option_int($options, 2, 0);
        $cat_limit_recursive = $this->_get_option_int($options, 3, 0);
     ...
     * @return string
     */
    public function build_where_block_cat_limitation($param)
    {
     ...
     * @return string
     */
    public function _get_catselbox($preset_id = 0, $none = 0, $sel_name = '', $onchange = '')
    {
     ...
     * @return array
     */
    public function _get_popbox_js($mode, $flag_popbox)
    {
        $show = false;
        $popbox_js = null;
        ...
        $popbox_js = $this->_header_class->assign_or_get_popbox_js($this->_constant('POPBOX_REVERT'));
     ...
     * @param $dirname
     */
    public function _init_xoops_config_for_block($dirname)
    {
        $configHandler = webphoto_inc_config::getSingleton($dirname);

        $this->_cfg_use_pathinfo = $configHandler->get_by_name('use_pathinfo');
        $this->_cfg_use_popbox = $configHandler->get_by_name('use_popbox');
        $this->_cfg_thumb_width = $configHandler->get_by_name('thumb_width');
        $this->_cfg_thumb_height = $configHandler->get_by_name('thumb_height');
        $this->_cfg_cat_main_width = $configHandler->get_by_name('cat_main_width');
        $this->_cfg_cat_sub_width = $configHandler->get_by_name('cat_sub_width');
        $this->_cfg_gmap_apikey = $configHandler->get_by_name('gmap_apikey');
        $this->_cfg_gmap_latitude = $configHandler->get_by_name('gmap_latitude');
        $this->_cfg_gmap_longitude = $configHandler->get_by_name('gmap_longitude');
        $this->_cfg_gmap_zoom = $configHandler->get_by_name('gmap_zoom');
        $this->_cfg_timeline_dirname = $configHandler->get_by_name('timeline_dirname');
    }
