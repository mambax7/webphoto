...
class webphoto_inc_xoops_version extends webphoto_inc_base_ini
{
    ...
    /**
     * webphoto_inc_xoops_version constructor.
     * @param $dirname
     * @param $trust_dirname
     */
    ...
    public function build_modversion()
    {
        ...
        $xReferer = $xSecurity->checkReferer();
        if ($this->_is_module_admin && $xReferer && ('post' == mb_strtolower($_SERVER['REQUEST_METHOD']))) {
    ...
    public function _build_notification()
    {
        ...
        $arr['category'][1]['subscribe_from'] = ['index.php'];
        ...
        $arr['category'][2]['subscribe_from'] = ['index.php'];
        ...
        $arr['category'][3]['subscribe_from'] = ['index.php'];
    ...
    public function _build_sub()
    {
        ...
        if ($this->_has_insertable) {
            $arr[] = $this->_build_sub_array_const('SMNAME_SUBMIT', $this->_build_sub_url_fct('submit'));

            if ($this->_ini_sub_myphoto) {
                $arr[] = $this->_build_sub_array_const('SMNAME_MYPHOTO', $this->_build_sub_url_fct('myphoto'));
            }
        }

        if ($this->_ini_sub_popular) {
            $arr[] = $this->_build_sub_array_const('SMNAME_POPULAR', $this->_build_sub_url_op('popular'));
        }

        if ($this->_ini_sub_highrate && $this->_has_rateview) {
            $arr[] = $this->_build_sub_array_const('SMNAME_HIGHRATE', $this->_build_sub_url_op('highrate'));
        }
        ...
        if ($this->_ini_sub_search) {
            $arr[] = $this->_build_sub_array_const('SMNAME_SEARCH', $this->_build_sub_url_op('search'));
        }
    ...
    public function _build_blocks()
    {
        ...
        if (!defined('XOOPS_CUBE_LEGACY') && mb_substr(XOOPS_VERSION, 6, 3) < 2.1 && !empty($_POST['fct']) && !empty($_POST['op']) && 'modulesadmin' == $_POST['fct'] && 'update_ok' == $_POST['op']
            && $_POST['dirname'] == $this->_DIRNAME) {
    ...
    {
        $this->_catlist_class = webphoto_inc_catlist::getSingleton($this->_DIRNAME, $this->_TRUST_DIRNAME);
    ...
    public function _init_config($dirname)
    {
        ...
        $this->_cfg_catonsubmenu = $configHandler->get_by_name('catonsubmenu');
    ...
    public function _init_group_permission($dirname, $trust_dirname)
    {
        ...
        $this->_has_insertable = $permissionHandler->has_perm('insertable');
        $this->_has_rateview = $permissionHandler->has_perm('rateview');
    }
    ...
    public function _init_ini($dirname, $trust_dirname)
    {
        ...
        $this->_ini_cfg_groupid_admin = $this->get_ini('xoops_version_cfg_groupid_admin');
        $this->_ini_cfg_groupid_user = $this->get_ini('xoops_version_cfg_groupid_user');
        $this->_ini_sub_myphoto = $this->get_ini('xoops_version_sub_myphoto');
        $this->_ini_sub_popular = $this->get_ini('xoops_version_sub_popular');
        $this->_ini_sub_highrate = $this->get_ini('xoops_version_sub_highrate');
        $this->_ini_sub_search = $this->get_ini('xoops_version_sub_search');
        $this->_ini_sub_cat_prefix = $this->get_ini('xoops_version_sub_cat_prefix');
    }
