...
if (defined('FOR_XOOPS_LANG_CHECKER') || !defined($constpref . 'LANG_LOADED')) {
    ...
    define($constpref . 'CFG_DESCFORCEGD2',
           '強制的にGD2モードで動作させます<br />一部のPHPでは強制GD2モードでサムネイル作成に失敗します<br />画像処理パッケージとしてGDを選択した時のみ意味を持ちます');
    define($constpref . 'CFG_IMAGICKPATH', 'ImageMagickの実行パス');
    define($constpref . 'CFG_DESCIMAGICKPATH',
           'convertの存在するディレクトリをフルパスで指定しますが、空白でうまく行くことが多いでしょう。<br />画像処理パッケージとしてImageMagickを選択した時のみ意味を持ちます');
    define($constpref . 'CFG_NETPBMPATH', 'NetPBMの実行パス');
    define($constpref . 'CFG_DESCNETPBMPATH',
           'pnmscale等の存在するディレクトリをフルパスで指定しますが、空白でうまく行くことが多いでしょう。<br />画像処理パッケージとしてNetPBMを選択した時のみ意味を持ちます');
    ...
    define($constpref . 'CFG_DESCWIDTH',
           'ポップアップしたときの画像の大きさです。<br /><br />従来のバージョンではアップロード可能な画像の大きさの制限でした。<br />この制限はなくなりました。<br />2.30にて仕様が変わりました。');
    ...
    define($constpref . 'CFG_DESCUSESITEIMG',
           'イメージマネージャ統合で、[img]タグの代わりに[siteimg]タグを挿入するようになります。<br />利用モジュール側で[siteimg]タグが有効に機能するようになっている必要があります');
    ...
    define($constpref . 'CFG_DESCFFMPEGPATH',
           'ffmpeg の存在するディレクトリをフルパスで指定します、空白でうまく行くことが多いでしょう。<br />「ffmpeg を使用する」の「はい」を選択した時のみ意味を持ちます');
...
FTP パスワード: xxx<br />');
    ...
    define($constpref . 'CFG_XPDFPATH_DSC',
           'pdftoppm などの存在するディレクトリをフルパスで指定します、空白でうまく行くことが多いでしょう。<br />「xpdf を使用する」の「はい」を選択した時のみ意味を持ちます');
    ...
    define($constpref . 'CFG_TIMIDITYPATH_DSC',
           'timidity の存在するディレクトリをフルパスで指定します、空白でうまく行くことが多いでしょう。<br />「timidity を使用する」の「はい」を選択した時のみ意味を持ちます');
