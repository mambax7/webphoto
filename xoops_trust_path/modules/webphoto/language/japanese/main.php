...
登録されない場合は、<a href="{MODULE_URL}/i.php?op=post" target="_blank">「メールから投稿する」</a> をクリックしてください。<br />');
    ...
    define('_WEBPHOTO_FLASHVAR_DISPLAYWIDTH_DSC',
           '[Playlist] [mediaplayer] <br />プレイリストを下に表示:<br /> スクリーン幅 = プレイヤー幅<br />プレイリストを横に表示<br />スクリーン幅 &gt; プレイヤー幅 ');
    ...
    define('_WEBPHOTO_FLASHVAR_FALLBACK_DSC',
           '[Playback] [mediaplayer] <br />もし MP4 を再生するならば、フォールバックする FLV の URL を設定すること <br />古いバージョンのフラッシュ・プレイヤーでは自動的に選択される ');
    ...
    define('_WEBPHOTO_FLASHVAR_ENABLEJS_DSC',
           '[External] <br />「はい」のとき、JavaScript による対話制御を有効にする <br />オンラインのとき動作する<br />JavaScript による対話制御には、コントロールバーや、メディアファイルのローデングの同期や、トラック情報の返信などを含む ');
    define('_WEBPHOTO_FLASHVAR_JAVASCRIPTID_DSC',
           '[External] <br />もしも、複数の mediaplayers/imagerotators と JavaScript による対話制御を行うときは、この項目にそれぞれに対するユニークな ID を設定する ');
    define('_WEBPHOTO_FLASHVAR_LINK_TYPE_DSC',
           '[External] <br />スクリーンやロゴやリンク・ボタンに割当されるリンクの種別を設定する<br /> 「なし」のときは、何もしない <br />それ以外は、そのリンクを割当てる');
    ...
    define('_WEBPHOTO_FLASHVAR_SEARCHLINK_DSC',
           '[External] [mediaplayer] <br />検索バーの検索先を設定する <br />デフォルトは「search.longtail.tv」である <br />検索バーを隠すには「検索バー」を使用する ');
    define('_WEBPHOTO_FLASHVAR_STREAMSCRIPT_DSC',
           "[External] [mediaplayer] <br />ストリーミング配信に使用するスクリプトの URL を設定する <br />パラメータ・ファイルはスクリプトへ送信される <br />もし LigHTTPD ストリーミング を使用するならば、'lighttpd' と設定する . ");
    define('_WEBPHOTO_FLASHVAR_TYPE_DSC',
           '[External] [mediaplayer] <br />mediaplayer は「file」の最後の３文字を拡張子を見なしている <br />データベースの ID や mod_rewrite などでうまく動作しない場合には、正しい拡張子を設定すること <br />もしも、確かでないときは、プレイリストがロードされたと見なす。');
    ...
    define('_WEBPHOTO_FLASHVAR_BUFFERLENGTH_DSC',
           '[Behaviour] bufferlength <br />再生する前に、動画をバッファする秒数を設定する<br />高速な通信環境や短い動画のときは小さい値を設定する<br />低速な通信環境のときは大きい値を設定する ');
    ...
    define('_WEBPHOTO_FLASHVAR_OVERSTRETCH_DSC',
           '[Appearance] overstretch <br />画像/動画をスクリーンの大きさに拡張する方法を設定する<br />false (デフォルト) = スクリーンに一致する<br />true = 均一に拡張する<br />fit = 不均一に拡張する<br />none = 元の大きさを保持する');
    ...
    define('_WEBPHOTO_FLASHVAR_AUDIO_DSC',
           '[Behaviour] audio <br />同期した MP3 を割り当てる<br />mediaplayer の音声説明またはディレクターのコメント、あるいは imagerotator の背景音楽として、使用する<br />mediaplayer とプレイリストのときは、エントリ毎にオーディオを設定できる ');
    ...
    define('_WEBPHOTO_FLASHVAR_PROVIDER_DSC',
           '[Playlist] provider <br />メディアの形式を設定する。<br />デフォルトでは、ファイル拡張子に基づき自動的に検出されます。<br />拡張子が全くなければ、手動でそれを設定します。');
    ...
    define('_WEBPHOTO_FLASHVAR_PLAYLIST_POSITION_DSC',
           '[Layout] playlist.position <br />プレイリストを表示する位置を指定します。<br />bottom, top, right, left, over or none.<br />デフォルトは 表示しない none です。');
    define('_WEBPHOTO_FLASHVAR_PLAYLIST_SIZE_DSC',
           '[Layout]  playlist.size <br />プレイリストをディスプレイの下に指定したときは、高さを変えるのにこのオプションを使用できます。 <br />プレイリストがディスプレイの左また右に指定したときは、このオプションは、幅を表します。<br />その他のときは、このオプションは必要ではありません。');

    define('_WEBPHOTO_FLASHVAR_PLAYER_REPEAT_DSC',
           '[Behaviour] repeat <br />none プレイリストの最初のファイル<br />list プレイリストを最初から最後まで<br />always プレイリストの繰り返し<br />single プレイリストの同じファイルの繰り返し');
    ...
    define('_WEBPHOTO_FLASHVAR_PLAYERREADY_DSC',
           '[Behaviour] playerready <br />それが初期化されるとき、デフォルトでは、プレーヤーは初期化されるときに、JavaScript の playerReady() を呼びます。<br />初期化された後に、プレーヤーに異なった機能を呼ばせるのに、このオプションは使用されます。');
    define('_WEBPHOTO_FLASHVAR_PLUGINS_DSC',
           '[Behaviour] plugins <br />これはロードするプラグインのコンマで切り離されたリストです。<br />プレーヤーの機能性を広げる別々の JavaScript か SWF ファイルです。<br />addons レポジトリを訪問して、利用可能なプラグインを見てください。');
    define('_WEBPHOTO_FLASHVAR_STRETCHING_DSC',
           '[Behaviour] stretching <br />画像/動画をスクリーンの大きさに拡張する方法を設定する<br />none: 原寸を保ちます。<br />exactfit: 不比例にビデオ/イメージを伸ばして、ディスプレイにぴったり合わせる。<br />uniform: アスペクトレシオを維持して、イメージ/ビデオを拡大します。黒い境界が表示されます。<br />fill: アスペクトレシオを維持して、イメージ/ビデオを拡大します。ディスプレイいっぱいに拡大します。 ');
    ...
    define('_WEBPHOTO_FLASHVAR_LOGO_POSITION_DSC',
           '[Logo] logo.position <br />ロゴを表示する四隅を指定します。<br />bottom-left, bottom-right, top-left, top-right.<br />デフォルトは左下 bottom-left です。');
